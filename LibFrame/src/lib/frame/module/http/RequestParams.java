...
//    }
    ...
    public RequestBody getRequestBody() {
        ...
        getRequestBody(builder, null, urlParamsWithObjects);
        return builder.build();
    }

    public void getRequestBody(FormBody.Builder builder, String key, Object value) {
        if (value instanceof Map) {
            ...
            for (Object nestedKey : list) {
                if (nestedKey instanceof String) {
                    ...
                    if (nestedValue != null) {
                        getRequestBody(builder, key == null ? (String) nestedKey : String.format(Locale.US, "%s[%s]", key, nestedKey), nestedValue);
                    }
                }
            }
            ...
            for (int nestedValueIndex = 0; nestedValueIndex < listSize; nestedValueIndex++) {
                getRequestBody(builder, String.format(Locale.US, "%s[%d]", key, nestedValueIndex), list.get(nestedValueIndex));
            }
        } else if (value instanceof Object[]) {
            Object[] array = (Object[]) value;
            int arrayLength = array.length;
            for (int nestedValueIndex = 0; nestedValueIndex < arrayLength; nestedValueIndex++) {
                getRequestBody(builder, String.format(Locale.US, "%s[%d]", key, nestedValueIndex), array[nestedValueIndex]);
            }
        } else if (value instanceof Set) {
            Set set = (Set) value;
            for (Object nestedValue : set) {
                getRequestBody(builder, key, nestedValue);
            }
        } else {
    ...
    public String toString() {
        ...
        String tempResult = getObjString(null, urlParamsWithObjects);
    ...
    public String getObjString(String key, Object value) {
        ...
        if (value instanceof Map) {
            ...
            for (Object nestedKey : list) {
                if (nestedKey instanceof String) {
                    ...
                    if (nestedValue != null) {
                        if (sb.length() > 0)
                            sb.append("&");
                        sb.append(getObjString(key == null ? (String) nestedKey : String.format(Locale.US, "%s[%s]", key, nestedKey), nestedValue));
                    }
                }
            }
        } else if (value instanceof List) {
            ...
            for (int nestedValueIndex = 0; nestedValueIndex < listSize; nestedValueIndex++) {
                if (sb.length() > 0)
                    sb.append("&");
                sb.append(getObjString(String.format(Locale.US, "%s[%d]", key, nestedValueIndex), list.get(nestedValueIndex)));
            }
        } else if (value instanceof Object[]) {
            Object[] array = (Object[]) value;
            int arrayLength = array.length;
            for (int nestedValueIndex = 0; nestedValueIndex < arrayLength; nestedValueIndex++) {
                if (sb.length() > 0)
                    sb.append("&");
                sb.append(getObjString(String.format(Locale.US, "%s[%d]", key, nestedValueIndex), array[nestedValueIndex]));
            }
        } else if (value instanceof Set) {
            Set set = (Set) value;
            for (Object nestedValue : set) {
                if (sb.length() > 0)
                    sb.append("&");
                sb.append(getObjString(key, nestedValue));
            }
        } else {
            if (sb.length() > 0)
                sb.append("&");
