...
public class WgWebViewBase extends WebView {
    ...
    private final static String TAG = "webviewbase";
     ...
     */
    public WgWebViewBase(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        initializeOptions();
        loadMethods();
    ...
    public void initializeOptions() {
        ...
        if (Build.VERSION.SDK_INT > 7) {
            settings.setPluginState(WebSettings.PluginState
                    .valueOf(WebSettings.PluginState.ON_DEMAND.toString()));
        }
    ...
    public void loadUrl(String url) {
        if (!TextUtils.isEmpty(url)) {
            if (url.startsWith("javascript:")) {
                super.loadUrl(url);
                return;
            } else if (url.startsWith("http://") || url.startsWith("https://") || url.startsWith("file:///")) {// 有外链，优先跳转外链
                super.loadUrl(url);
            } else {
...
//        Log.i(TAG, progress + "");
        ...
        if (vProgressBar != null) {
            if ((progress - vProgressBar.getProgress()) > 10
                    && (int) (progress * 1.1) <= 99) {
                vProgressBar.setProgress((int) (progress * 1.1));
                vProgressBar.postInvalidate();
            }
            if (progress >= 90) {
    ...
    public boolean isSameUrl(String url) {
        return url != null && url.equalsIgnoreCase(this.getUrl());
    }
    ...
    public void setClientInterface(WgWebViewInterface wgWebViewInterface) {
        if (mWgWebViewClientBase == null)
            mWgWebViewClientBase = new WgWebViewClientBase(mContext, wgWebViewInterface);
        if (mWgChromeClientBase == null)
            mWgChromeClientBase = new WgChromeClientBase((Activity) mContext, wgWebViewInterface);
...
//            }
//        });
        setWebViewClient(mWgWebViewClientBase);
        setWebChromeClient(mWgChromeClientBase);
    }
    ...
    public interface ScrollInterface {
        void onSChanged(WebView view, int l, int t, int oldl, int oldt);
    }
}
