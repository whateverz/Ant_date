...
public class WgSwitchButton extends CompoundButton {
    ...
    private final ThreadLocal<Layout> mOffLayout = new ThreadLocal<>();
    ...
    public WgSwitchButton(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init(attrs);
    }

    public WgSwitchButton(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }
    ...
    private void init(AttributeSet attrs) {
        ...
        TypedArray ta = attrs == null ? null : getContext().obtainStyledAttributes(attrs, R.styleable.WgSwitchButton);
        if (ta != null) {
            thumbDrawable = ta.getDrawable(R.styleable.WgSwitchButton_wgThumbDrawable);
            thumbColor = ta.getColorStateList(R.styleable.WgSwitchButton_wgThumbColor);
            margin = ta.getDimension(R.styleable.WgSwitchButton_wgThumbMargin, margin);
            marginLeft = ta.getDimension(R.styleable.WgSwitchButton_wgThumbMarginLeft, margin);
            marginRight = ta.getDimension(R.styleable.WgSwitchButton_wgThumbMarginRight, margin);
            marginTop = ta.getDimension(R.styleable.WgSwitchButton_wgThumbMarginTop, margin);
            marginBottom = ta.getDimension(R.styleable.WgSwitchButton_wgThumbMarginBottom, margin);
            thumbWidth = ta.getDimension(R.styleable.WgSwitchButton_wgThumbWidth, thumbWidth);
            thumbHeight = ta.getDimension(R.styleable.WgSwitchButton_wgThumbHeight, thumbHeight);
            thumbRadius = ta.getDimension(R.styleable.WgSwitchButton_wgThumbRadius, Math.min(thumbWidth, thumbHeight) / 2.f);
            backRadius = ta.getDimension(R.styleable.WgSwitchButton_wgBackRadius, thumbRadius + density * 2f);
            backDrawable = ta.getDrawable(R.styleable.WgSwitchButton_wgBackDrawable);
            backColor = ta.getColorStateList(R.styleable.WgSwitchButton_wgBackColor);
            backMeasureRatio = ta.getFloat(R.styleable.WgSwitchButton_wgBackMeasureRatio, backMeasureRatio);
            animationDuration = ta.getInteger(R.styleable.WgSwitchButton_wgAnimationDuration, animationDuration);
            fadeBack = ta.getBoolean(R.styleable.WgSwitchButton_wgFadeBack, true);
            tintColor = ta.getColor(R.styleable.WgSwitchButton_wgTintColor, tintColor);
            textOn = ta.getString(R.styleable.WgSwitchButton_wgTextOn);
            textOff = ta.getString(R.styleable.WgSwitchButton_wgTextOff);
            textMarginH = Math.max(textMarginH, backRadius / 2);
            textMarginH = ta.getDimension(R.styleable.WgSwitchButton_wgTextMarginH, textMarginH);
            autoAdjustTextPosition = ta.getBoolean(R.styleable.WgSwitchButton_wgAutoAdjustTextPosition, true);
            ta.recycle();
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mOffLayout.get() == null && mTextOff != null) {
            mOffLayout.set(makeLayout(mTextOff));
        }
    ...
    private int measureWidth(int widthMeasureSpec) {
        ...
        float offWidth = mOffLayout.get() != null ? mOffLayout.get().getWidth() : 0;
    ...
    private int measureHeight(int heightMeasureSpec) {
        ...
        float offHeight = mOffLayout.get() != null ? mOffLayout.get().getHeight() : 0;
    ...
    private void setup() {
        ...
        if (mOnLayout != null && mOffLayout.get() != null) {
        ...
        if (mOffLayout.get() != null) {
            float marginOffX = mBackRectF.right - (mBackRectF.width() - mThumbRectF.width() - mThumbMargin.left - mOffLayout.get().getWidth()) / 2 - mOffLayout.get().getWidth() + (mThumbMargin.right < 0 ? mThumbMargin.right * 0.5f : 0);
            if (!mIsBackUseDrawable && mAutoAdjustTextPosition) {
                marginOffX -= mBackRadius / 4;
            }
            float marginOffY = mBackRectF.top + (mBackRectF.height() - mOffLayout.get().getHeight()) / 2;
            mTextOffRectF.set(marginOffX, marginOffY, marginOffX + mOffLayout.get().getWidth(), marginOffY + mOffLayout.get().getHeight());
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // text
        Layout switchText = getProcess() > 0.5 ? mOnLayout : mOffLayout.get();
