package lib.frame.view.pickerview.lib;
...
import lib.frame.view.pickerview.adapter.WheelAdapter;
...
public class WheelView extends View {
    ...
    SparseArray<Object> visibles = new SparseArray<>();
    SparseArray<Integer> visibleItems = new SparseArray<>();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        try {
            ...
            Log.i("WheelView", "出错了！adapter.getItemsCount() == 0，联动数据不匹配");
        }
        if (!isLoop) {//不循环的情况
        ...
        while (counter < itemsVisible) {
            ...
            if (isLoop) {
                ...
                visibles.put(counter, adapter.getItem(index));
            } else if (index < 0) {
                visibles.put(counter, "");
            } else if (index > adapter.getItemsCount() - 1) {
                visibles.put(counter, "");
            } else {
                visibles.put(counter, adapter.getItem(index));
            }
            visibleItems.put(counter, index);
            counter++;
        }
        ...
        while (counter < itemsVisible) {
            ...
            if (angle >= 90F || angle <= -90F) {
                ...
                String contentText = getContentText(visibles.get(counter));
                ...
                if (translateY <= firstLineY && maxTextHeight + translateY >= firstLineY) {
                    ...
                    canvas.drawText(contentText, drawCenterContentStart, maxTextHeight - CENTERCONTENTOFFSET, paintCenterText);
                    int preSelectedItem = visibleItems.get(counter);
